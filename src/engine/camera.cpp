...
namespace Engine {
    ...
    void Camera::SetPerspectiveProjection(float verticalFov, float aspectRatio, float near, float far) {
        ...
        _projectionMatrix[0][0] = 1.0f / (aspectRatio * tanHalfVerticalFov);
        _projectionMatrix[1][1] = 1.0f / (tanHalfVerticalFov); // Vulkan uses -Y for up
        _projectionMatrix[2][2] = far / (far - near);
        _projectionMatrix[2][3] = 1.0f;
        _projectionMatrix[3][2] = -(near * far) / (far - near);
    }

    void Camera::SetViewDirection(glm::vec3 position, glm::vec3 direction, glm::vec3 up) {
        assert(direction != glm::vec3(0.0f) && "View direction cannot be (0, 0)");
        ...
        _inverseViewMatrix = glm::mat4{ 1.0f };
